...
for(int i = 1; i <= Chain.Length();i++ )
  ...
  switch(current)
	...
	case N0:
	  if(Chain[i]=='x')
		{
		Memo1->Lines->Add("Переходим в состояние 2");
		current = N1;
		way[i-1]=2;
		}
	  if(Chain[i]=='y')
		{
		Memo1->Lines->Add("Переходим в состояние 4");
		current = N3;
		way[i-1]=3;
		}
    break;
	case N1:
	  if(Chain[i]=='x')
        {
		Memo1->Lines->Add("Переходим в состояние 5");
		current = N4;
		way[i-1]=4;
		}
	  if(Chain[i]=='y')
		{
		Memo1->Lines->Add("Переходим в состояние 3");
		current = N2;
		way[i-1]=5;;
        }
    break;
	...
	case N4:
	  if(Chain[i]=='x')
		{
		Memo1->Lines->Add("Переходим в состояние 1");
		current = N0;
		way[i-1]=10;
        }
	  if(Chain[i]=='y')
...
j=0;
if(current==N4)
  ShowMessage("Цепочка пренадлежит языку");
  else
  ShowMessage("Цепочка не пренадлежит языку");
}
}
//---------------------------------------------------------------------------
